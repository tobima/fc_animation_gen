...
public class TextureImage implements Runnable {
	 ...
	 * texture on the backing canvas. This is asynchronous, the method will
	 * return immediately, but the rendering process will continue on background
	 * threads. Use RenderAndWait to only return when the rendering is
	 * completed.
	 * 
	 * @param texture
	 *            Texture to render
	...
	/**
	 * This method starts the rendering process and waits until it is completed
	 * before returning. This is so we can render a texture and then immediately
	 * save it.
	 * 
	 * @param texture
	 *            Texture to render
	 */
	public void renderAndWait(Texture texture) {
		render(texture);
		while (threadCount > 0) {
			try {
				Thread.sleep(50);
